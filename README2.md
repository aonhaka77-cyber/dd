...
<body>
    ...
    <section class="max-w-xl mx-auto p-6">
        <div class="main-card rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-premium">
            <div class="relative z-10">
                <div class="flex justify-between items-start mb-10">
                    <div>
                        <span class="text-slate-400 text-[10px] font-bold uppercase tracking-widest">현재 총 잔액</span>
                        <h2 id="total-balance" class="text-4xl font-black mt-1 tabular-nums">0원</h2>
                    </div>
                    <button onclick="App.actions.setBudget()" class="bg-indigo-500/20 px-3 py-1.5 rounded-xl border border-indigo-500/30 active-push">
                ...
                <div class="w-full bg-white/5 h-2 rounded-full overflow-hidden mb-8 border border-white/5">
                    <div id="budget-bar" class="h-full bg-indigo-500 transition-all duration-1000" style="width: 0%"></div>
                </div>
    ...
    <section class="max-w-xl mx-auto px-6 mb-6">
        <div class="flex gap-2">
            ...
            <button onclick="App.actions.exportCSV()" class="px-4 bg-white border border-slate-100 rounded-2xl text-slate-400 active-push">
    ...
    <div class="fixed bottom-8 left-0 right-0 z-40 px-6 flex justify-center items-center gap-4 pointer-events-none">
        <label class="pointer-events-auto w-16 h-16 bg-white border-2 border-slate-100 rounded-2xl shadow-2xl flex items-center justify-center active-push cursor-pointer">
        ...
        <button onclick="App.ui.openModal()" class="pointer-events-auto flex-1 max-w-[240px] h-16 bg-slate-900 text-white rounded-2xl shadow-2xl shadow-slate-200 font-bold flex items-center justify-center gap-3 active-push">
            <i data-lucide="plus" class="w-6 h-6"></i>
            <span class="text-lg">내역 추가하기</span>
        </button>
    </div>
    ...
    <div id="input-modal" class="hidden fixed inset-0 z-50 flex items-end justify-center">
        ...
        const App = {
            ...
            actions: {
                ...
                async delete(id) {
                    ...
                    if (!error) await App.actions.fetch();
                },
                ...
                setBudget() {
                    const val = prompt("목표 월 예산을 입력하세요.", App.state.budget);
                    if (val && !isNaN(val)) {
                        App.state.budget = parseInt(val);
                        localStorage.setItem('sb_ledger_budget', App.state.budget);
                        App.ui.renderList();
                    }
                },
                ...
                exportCSV() {
                    if (App.state.items.length === 0) return;
                    let csv = "날짜,내역,금액\n";
                    App.state.items.forEach(r => csv += `${r.date},${r.description},${r.amount}\n`);
                    const blob = new Blob(["\uFEFF" + csv], { type: 'text/csv;charset=utf-8;' });
                    const url = URL.createObjectURL(blob);
                    const a = document.createElement('a');
                    a.href = url;
                    a.download = `가계부_백업_${new Date().toISOString().slice(0,10)}.csv`;
                    a.click();
                }
            },
            ...
            ui: {
                renderList() {
                    ...
                    // 예산 시각화
                    const percent = Math.min(Math.round((exp / App.state.budget) * 100), 100);
                    document.getElementById('budget-bar').style.width = percent + '%';
                    document.getElementById('budget-info').innerText = `지출 ${percent}% (목표 ${App.state.budget.toLocaleString()})`;
                ...
                toggleCategoryModal() {
                    ...
                    if(!m.classList.contains('hidden')) this.renderCatManageList();
                },

                toggleStats() {
                    const v = document.getElementById('stats-view');
                    v.classList.toggle('hidden');
                    if (!v.classList.contains('hidden')) this.renderChart();
                },
